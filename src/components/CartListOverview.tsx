...
const CartListOverview = ({ onSetOpen, item }: CartListOverviewProps) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 z-30">
      <div className="h-[600px] w-[90%] max-w-[425px] bg-white rounded-md border shadow-sm relative top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 p-5 z-40 overflow-y-auto">
        <button
          onClick={() => onSetOpen(false)}
          className="absolute top-4 right-4 text-3xl text-gray-600 transition hover:scale-110 hover:text-red-600 font-medium w-7 h-7"
        >
          &times;
        </button>
